...
public class Application3 {
    ...
    public static void main(String[] args){
        ...
        *    1-2  예약어는 사용이 붤가능하다.(자바에서 미리 사용하겠다고 컴파일러와 약속한 키워드를 예약이라고 한다
        *    1-3 변수명은 대/소문자를 구분한다(자바는 대/소문자를 구분하는 언어이다)
        *    1-4 변수명은 숫자로 시작할 수 없다(시작만 할 수 없고 숫자를 섞어서 사용하는 것은 가능하다)
        *    1-5 특수기호는 '_'와 '&'만 사용 가능하다(다라서 공벽은 사용할 수 없다.)
        * 2. 컴파일 에러를 발생시키지는 않지만 개발자끼리의 암묵적인규칙
        ...
        *    2-5 변수 안에 저장된 값이 어떤 의미리르 가지는지 정확하게 표현하도록 한다.
        *    2-6 전형적인 변수 이름이 있따면 가급적 사용하도록 한다.
        *    2-7 명사형으로 작성하도록 한다.
        ...
        *     */
        ...
        int ahe = 20;
        //string age = fagikjbhasfo;
        //int ahe = 30; //동일한 변수명을 가지므로 에러 발생
        ...
        int a_$ge =20; //$사용 가능함. 처음도 가능하고 중간이나 끝에도 가능함

        /*2. 컴파일 에러를 발생시키지는 않지만 개발자들끼리의 암묵적인 규칙*/
        /* 목차 2-1 변수명의 길이제한은 없다. 하지만 적당히 하자.*/
        int fkdsalfkmdsakfdsanlkjfddddddddddddddddddddddddddddddddddddddddddds;

        /* 목차 2-2 변수명이 합성어로 이루어진 경우 첫 단어는 소문자 두 번째 시작 단어는 대문자로 시작한다*/
        /* 자바에서는 ㅋ틀래스명만 유일하게 대문자로 시작한다.*/
        int maxAge = 20;
        int minAge = 20;
        ...
        /* 목차 2-6. 정형적인 변수 이름이 있다면 가급적 사용하도록 한다*/
        int sum = 0;
        int max = 10;
        int min = 0;
        int count = 1;
